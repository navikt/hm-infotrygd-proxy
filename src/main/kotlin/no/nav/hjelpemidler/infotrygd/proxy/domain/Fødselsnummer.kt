 ...
 */
private fun byttFormat(value: String): String = "${value.substring(4, 6)}${value.substring(2, 4)}${value.substring(0, 2)}${value.substring(6)}"

fun fødselsnummerFraInfotrygd(value: String): Fødselsnummer = Fødselsnummer(byttFormat(value))

fun Row.fødelsnummer(columnName: String): Fødselsnummer = fødselsnummerFraInfotrygd(string(columnName))
